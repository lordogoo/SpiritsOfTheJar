...
public class transitionfadeoutanimation2 extends animation
{

    public transitionfadeoutanimation2(gamepanel gp,triggeranimations t,int durration,Color b,Vector<JComponent> cl)
    {
    ...
    public void update(){
        for(int i = 0; i < target.numcontainers();i++){   
            if(currentframe <= framesdurration){
                target.getcomponentcontainer(i).setopacity((1.f-(((float)currentframe)/((float)framesdurration))));
            }else{
